...
PanelWindow {
    ...
    readonly property int topBarClearance: scaler.s(56)
    ...
    function setScreenByName(name) {
        let scr = screenByName(name);
        if (scr) masterWindow.screen = scr;
    }

    function syncFocusedMonitor() {
        focusedMonitorProc.running = false;
        focusedMonitorProc.running = true;
    }

    function openAfterScreenSync() {
        screenSyncFallback.stop();
        ...
        executeSwitch(w, a, false);
    }

    Process {
        id: focusedMonitorProc
        command: ["bash", "-c", "export PATH=$PATH:/run/current-system/sw/bin && hyprctl monitors -j | jq -r '.[] | select(.focused==true) | .name' | head -n1"]
        stdout: StdioCollector {
            onStreamFinished: {
                let name = this.text.trim();
                if (name.length > 0) masterWindow.setScreenByName(name);
                if (masterWindow.pendingWidget !== "") masterWindow.openAfterScreenSync();
            }
        }
    }
    ...
    Socket {
        ...
        parser: SplitParser {
            onRead: msg => {
                let match = /^focusedmon>>([^,]+)/.exec(msg);
                if (match) masterWindow.setScreenByName(match[1]);
            }
        }
    }

    Timer {
        id: screenSyncFallback
        interval: 120
        repeat: false
        onTriggered: masterWindow.openAfterScreenSync()
    }
    ...
    Item {
        ...
        height: masterWindow.topBarClearance

        anchors.leftMargin: (masterWindow.currentActive !== "hidden" && masterWindow.animX < 10 && masterWindow.animY < masterWindow.topBarClearance) ? masterWindow.animW : 0
        anchors.rightMargin: (masterWindow.currentActive !== "hidden" && (masterWindow.animX + masterWindow.animW) > (parent.width - 10) && masterWindow.animY < masterWindow.topBarClearance) ? masterWindow.animW : 0
    ...
    Component.onCompleted: {
        masterWindow.syncFocusedMonitor();
    ...
    function executeSwitch(newWidget, arg, immediate) {
        ...
        if (newWidget === "wallpaper") {
            Qt.callLater(function() {
                ...
                if (item && item.reloadLocalWallpapers) item.reloadLocalWallpapers();
            });
        }
    }
