...
Item {
    ...
    property int layoutWidth: parent ? parent.width : Screen.width
    property int layoutHeight: parent ? parent.height : 0
    ...
    Scaler {
        id: scaler
        currentWidth: window.layoutWidth > 0 ? window.layoutWidth : (parent ? parent.width : Screen.width)
    }
    ...
    Rectangle {
        anchors.fill: parent
        color: Qt.rgba(_theme.base.r, _theme.base.g, _theme.base.b, 0.9)
        z: -1
    }
    ...
    property bool isListing: false

    property bool isStartup: localFolderModel.status === FolderListModel.Loading && localProxyModel.count === 0
    property bool isReady: localProxyModel.count > 0 || localFolderModel.status === FolderListModel.Ready
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    ( matugen image "$FINAL_THUMB" --mode "$(cat ~/.config/hypr/theme_mode 2>/dev/null || echo dark)" || true; bash "$RELOAD_SCRIPT" || true ) &
                `;
        ...
        const fullScript = `
            ...
            ${wallpaperCmd}
            ( matugen image "${escThumb}" --mode "$(cat ~/.config/hypr/theme_mode 2>/dev/null || echo dark)" || true; bash "${escReload}" || true ) &
        `;
    ...
    function normalizeFileUrl(urlOrPath) {
        if (urlOrPath === undefined || urlOrPath === null || urlOrPath === "") return "";
    ...
    function finishLocalListing() {
        window.isListing = false;
    ...
    function reloadLocalWallpapersNow() {
        ...
        Quickshell.execDetached(["bash", window.scriptsDir + "/wallpaper_prep.sh"]);

        // FolderListModel won't re-fire onCountChanged after a clear if the folder is unchanged.
        Qt.callLater(function() {
            window.syncLocalModel();
            if (localProxyModel.count > 0) {
                window.finishLocalListing();
            }
        });

        listingFallbackTimer.restart();
        localListProcess.running = false;
        localListProcess.running = true;
    }
    ...
    Timer {
        ...
        interval: 80
        repeat: false
        onTriggered: window.reloadLocalWallpapersNow()
    }

    Timer {
        id: listingFallbackTimer
        interval: 2500
        repeat: false
        onTriggered: {
            if (!window.isListing) return;
            window.syncLocalModel();
            window.finishLocalListing();
        }
    }
    ...
    Process {
        ...
        command: ["bash", "-c",
            ...
            "bash '" + window.scriptsDir + "/wallpaper_list.sh' >/dev/null && " +
            "cat '" + paths.getCacheDir("wallpaper_picker") + "/wallpapers.json' 2>/dev/null || echo '[]'"
        ]
        stdout: StdioCollector {
            onStreamFinished: {
                window.isModelChanging = true;
                ...
                try {
                    ...
                    if (text.length > 0) {
                        ...
                        if (Array.isArray(items)) {
                            for (let i = 0; i < items.length; i++) {
                                let fn = items[i].fileName;
                                let fu = items[i].fileUrl;
                                if (fn) {
                                    batch.push({
                                        "fileName": String(fn),
                                        "fileUrl": window.normalizeFileUrl(fu)
                                    });
                                }
                            }
                        }
                    ...
                    console.log("[WallpaperPicker] wallpapers.json parse error:", e);
                }

                if (batch.length > 0) {
                    localProxyModel.append(batch);
                    window._localSyncedCount = batch.length;
                } else {
                    window.syncLocalModel();
                }

                window.isModelChanging = false;
                listingFallbackTimer.stop();
                window.finishLocalListing();
        ...
        onExited: function(exitCode) {
            if (window.isListing) {
                console.log("[WallpaperPicker] wallpaper list loader exited with code " + exitCode);
                window.syncLocalModel();
                listingFallbackTimer.stop();
                window.finishLocalListing();
            }
    ...
    Timer {
        ...
        interval: 2000
        repeat: false
        onTriggered: window.reloadLocalWallpapers()
    }
    ...
    onVisibleChanged: {
        if (!visible) {
            ...
            window.reloadLocalWallpapers();
            prepRepollTimer.restart();
    ...
    property bool isLoading: window.isListing ||
                             localFolderModel.status === FolderListModel.Loading ||
    ...
    onWidgetArgChanged: {
        if (widgetArg !== "") {
            ...
            Qt.callLater(function() { window.tryFocus(); });
        }
    }
    ...
    readonly property string srcDir: {
        const fromConfig = (typeof Config !== "undefined" && Config.wallpaperDir) ? Config.wallpaperDir : "";
        if (fromConfig && fromConfig !== "") return fromConfig.replace(/\/$/, "");
        const fromEnv = Quickshell.env("WALLPAPER_DIR");
        ...
        if (folderCount > window._localSyncedCount) {
            ...
            for (let i = window._localSyncedCount; i < folderCount; i++) {
                ...
                let fu = localFolderModel.get(i, "fileUrl")
                    || localFolderModel.get(i, "fileURL")
                    || localFolderModel.get(i, "filePath");
                if (fn !== undefined) {
                    batch.push({ "fileName": fn, "fileUrl": window.normalizeFileUrl(fu) });
                }
            }
    ...
    ListView {
        ...
        opacity: (localProxyModel.count > 0 || window.isReady) ? 1.0 : 0.0
        anchors.margins: (localProxyModel.count > 0 || window.isReady) ? 0 : window.s(40)
