...
# API Settings from .env (grep-based: resilient to malformed lines from partial writes)
read_env_var() {
    local key="$1"
    if [ ! -f "$ENV_FILE" ]; then
        return
...
KEY="$(read_env_var OPENWEATHER_KEY)"
ID="$(read_env_var OPENWEATHER_CITY_ID)"
UNIT="$(read_env_var OPENWEATHER_UNIT)"
UNIT="${UNIT:-metric}"
...
get_data() {
    ...
    # ---------------------------------------------------------
    if [[ -z "$KEY" || "$KEY" == "Skipped" || "$KEY" == "OPENWEATHER_KEY" || -z "$ID" ]]; then
        write_dummy_data
        return
    fi
